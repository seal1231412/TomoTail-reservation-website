...
function renderReservationList() {
	...
	reservationItems.innerHTML = `<div class="reservation-table-wrap"><table class="reservation-table"><thead><tr><th>Date</th><th>Name</th><th>Phone number</th><th>Amount of dogs</th><th>Weight of dogs</th><th>Time</th><th>Status</th><th>Notes</th><th>Decision</th></tr></thead><tbody>${reservations.map(([reservationId, entries]) => {
		...
		const decision = first.status === 'pending' ? `<div class="reservation-actions"><button class="confirm-reservation" type="button" data-status-id="${escapeHTML(reservationId)}" data-status="confirmed">Confirm</button><button class="deny-reservation" type="button" data-status-id="${escapeHTML(reservationId)}" data-status="cancelled">Deny</button></div>` : '<span class="reservation-meta">Decision complete</span>';
		return `<tr><td>${dates}</td><td>${escapeHTML(first.name || 'Test reservation')}</td><td>${escapeHTML(first.phone || 'Not provided')}</td><td>${dogsIn(first)}</td><td>${escapeHTML(weights)}</td><td>${escapeHTML(first.dropOff || 'Not selected')} - ${escapeHTML(first.pickUp || 'Not selected')}</td><td class="status">${escapeHTML(first.status)}</td><td>${escapeHTML(first.notes || 'None')}</td><td>${decision}</td></tr>`;
